...
// Venue photos for 2026 (static assets, not in S3)
...
const venuePhotos = [
  { id: 'venue-1', url: venueExterior, label: "Little Woodrow's EaDo" },
  { id: 'venue-2', url: venueInterior, label: 'Inside' },
  { id: 'venue-3', url: draftLocation, label: 'Our Draft Location' },
]

// Lightbox state for venue photos
const venueLightboxOpen = ref(false)
const venueLightboxIndex = ref(0)

const openVenueLightbox = (index) => {
  venueLightboxIndex.value = index
  venueLightboxOpen.value = true
}

const closeVenueLightbox = () => {
  venueLightboxOpen.value = false
}
  ...
  <div class="py-8 px-4">
    <div class="max-w-4xl mx-auto">
      ...
      <template v-else>
        ...
        <!-- Venue Photos (2026 only) -->
        <div v-if="year === 2026" class="bg-white rounded-lg shadow-lg p-6 mb-6">
          <h2 class="text-xl font-bold text-gray-900 mb-4">Venue: Little Woodrow's EaDo</h2>
          <div class="grid grid-cols-3 gap-2">
            <div
              v-for="(photo, index) in venuePhotos"
              :key="photo.id"
              class="relative cursor-pointer group rounded overflow-hidden"
              @click="openVenueLightbox(index)"
            >
              <img
                :src="photo.url"
                :alt="photo.label"
                class="w-full h-40 object-cover transition-opacity group-hover:opacity-90"
              />
        ...
        <PhotoLightbox
          v-if="year === 2026"
          :is-open="venueLightboxOpen"
          :photos="venuePhotos"
          :initial-index="venueLightboxIndex"
          @close="closeVenueLightbox"
        />
      </template>
    </div>
  </div>
